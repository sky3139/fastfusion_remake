...
FusionMipMapCPU::OwnMeshCellArray_ &FusionMipMapCPU::OwnMeshCellArray_::operator=(const FusionMipMapCPU::OwnMeshCellArray_ &value)
{
...
FusionMipMapCPU::FusionMipMapCPU(
	...
	: Fusion_AoS(0.0, 0.0, 0.0, scale, distanceThreshold, n, color),
...
#endif
	  _interactiveMemoryManagement(INTERACTIVE_MEMORY_MANAGEMENT), _meshThread(NULL),
	  //_threadMeshing(THREAD_MESHING)
	  _threadMeshing(false), _degenerate_faces(0), _mc(_brickLength, _brickLength), _treeinfo(NULL, _brickLength, _brickSize,
	  ...
	  _meshingDone(0), _meshSeparateCurrent(new MeshSeparate(3)), _meshSeparateNext(new MeshSeparate(3)), _meshCurrent(new MeshInterleaved(3)), _meshNext(new MeshInterleaved(3)), _updateCurrent(new CellUpdate()), _updateNext(new CellUpdate())

	  ,
	  _loopClosureLogging(false), _loopClosureEnabled(false), _loopClosureDone(true), _maxPosesPerLoop(MAX_NUMBER_POSES_PER_LOOP), _nLeavesLoopTotal(0), _nLeavesLoopUsed(0), _loopClosureFusion(NULL), _imageSaveThread(NULL), _loopClosureThread(NULL), _loopClosureMode(false), _weightsLoop(NULL), _leafMapLoop(NULL), _nLeavesMapped(0), _numCamsPerLeaf(NULL), _maxImageBufferSize(MAX_NUMBER_POSES_PER_LOOP), _loggingEnabled(false), _meshingStartFrame(0), _meshTime(0.0), _numCheckImages(0)
{
	...
	// Available Memory
	long long availableMemory = cpuMemory;
	if (availableMemory > LAPPY_MEMORY)
		availableMemory = LAPPY_MEMORY;
...
FusionMipMapCPU::FusionMipMapCPU(FusionMipMapCPU *fusion, volumetype nLeavesTotalLoop)
	: Fusion_AoS(fusion->_offset.x, fusion->_offset.y, fusion->_offset.z, fusion->_scale, fusion->_distanceThreshold, fusion->_n, fusion->_useColor),
	  ...
	  _meshCells(_meshCellsCompact),
#endif
	  _interactiveMemoryManagement(false), _meshThread(NULL),
	  _threadMeshing(false), _degenerate_faces(0), _mc(_brickLength, _brickLength), _treeinfo(NULL, _brickLength, _brickSize,
																							  std::min((double)_framesAdded, MIN_WEIGHT_FOR_SURFACE),
																							  _offset, _scale, &_degenerate_faces, _nBranchesUsed, _nLeavesUsed, _tree,
																							  _leafPos, _leafScale, _distance, _weights, _color),
	  _meshingDone(0), _meshSeparateCurrent(NULL), _meshSeparateNext(NULL), _meshCurrent(NULL), _meshNext(NULL), _updateCurrent(new CellUpdate()), _updateNext(new CellUpdate())

	  ,
	  _loopClosureLogging(false), _loopClosureEnabled(false), _loopClosureDone(true), _maxPosesPerLoop(MAX_NUMBER_POSES_PER_LOOP), _nLeavesLoopTotal(0), _nLeavesLoopUsed(0), _loopClosureFusion(NULL), _imageSaveThread(NULL), _loopClosureThread(NULL), _loopClosureMode(true), _weightsLoop(NULL), _leafMapLoop(NULL), _nLeavesMapped(0), _numCamsPerLeaf(NULL), _maxImageBufferSize(MAX_NUMBER_POSES_PER_LOOP), _numCheckImages(0)
{
...
FusionMipMapCPU::~FusionMipMapCPU()
{
	...
	if (_newBudsSinceMeshingToQueue.subtreeBuds)
		delete _newBudsSinceMeshingToQueue.subtreeBuds;
	if (_newBudsSinceMeshingToQueue.subtreeBudsParentLeaf)
		delete _newBudsSinceMeshingToQueue.subtreeBudsParentLeaf;
	if (_newBudsSinceMeshingToQueue.leafBuds)
		delete _newBudsSinceMeshingToQueue.leafBuds;

	if (_newBudsSinceMeshingToAccumulate.subtreeBuds)
		delete _newBudsSinceMeshingToAccumulate.subtreeBuds;
	if (_newBudsSinceMeshingToAccumulate.subtreeBudsParentLeaf)
		delete _newBudsSinceMeshingToAccumulate.subtreeBudsParentLeaf;
	if (_newBudsSinceMeshingToAccumulate.leafBuds)
		delete _newBudsSinceMeshingToAccumulate.leafBuds;

	if (_newBudsSinceMeshingToClear.subtreeBuds)
		delete _newBudsSinceMeshingToClear.subtreeBuds;
	if (_newBudsSinceMeshingToClear.subtreeBudsParentLeaf)
		delete _newBudsSinceMeshingToClear.subtreeBudsParentLeaf;
	if (_newBudsSinceMeshingToClear.leafBuds)
		delete _newBudsSinceMeshingToClear.leafBuds;
...
#ifdef BRANCHNEIGHBORHOOD_REFERECE
		if (_meshCellIndicesBranch[i])
			delete _meshCellIndicesBranch[i];
#else
		for (size_t j = 0; j < 6; j++)
		{
	...
	fprintf(stderr, "\nBranches and Leaves need %li MB memory",
			((long int)_nBranchesUsed * 16 * (long int)sizeof(volumetype) + (long int)_nLeavesUsed * ((long int)sizeof(sidetype3) + (long int)sizeof(sidetype) + (long int)sizeof(volumetype) + (long int)_brickSize * ((long int)sizeof(float) + (long int)sizeof(weighttype) + (long int)sizeof(colortype3)))) / (1024 * 1024));
	...
	{
		{
			...
			{
				logfileTime << i << " " << (_frameStatistics[i].timeSDFUpdate + _frameStatistics[i].timeTraversal) / cv::getTickFrequency() * 1000.0
	...
	if (_meshThread)
		_meshThread->join();
	freeMemoryCPU();
	if (_meshCellIsQueuedCurrent)
		delete _meshCellIsQueuedCurrent;
	if (_meshCellIsQueuedNext)
		delete _meshCellIsQueuedNext;
	if (_meshSeparateCurrent)
		delete _meshSeparateCurrent;
	if (_meshSeparateNext)
		delete _meshSeparateNext;

	if (_loopClosureFusion)
		delete _loopClosureFusion;
	if (_loopClosureThread)
		delete _loopClosureThread;

	if (_updateCurrent)
		delete _updateCurrent;
	if (_updateNext)
		delete _updateNext;
...
bool FusionMipMapCPU::allocateMemoryCPU()
{
	...
	for (volumetype i = 0; i < _nBranchesTotal; i++)
		_tree[i] = BRANCHINIT;
	int allocResult;
#ifdef OWNAVX
	...
	if (_loopClosureMode)
	{
		...
		for (volumetype i = 0; i < _nLeavesTotal; i++)
			_leafMapLoop[i] = BRANCHINIT;
	}
...
void FusionMipMapCPU::freeMemoryCPU()
{
	...
	if (_tree)
		delete[] _tree;
	if (_queueIndexOfLeaf)
		delete[] _queueIndexOfLeaf;
	if (_leafNumberSurface)
		delete[] _leafNumberSurface;
	if (_leafNumberFrustum)
		delete[] _leafNumberFrustum;
	if (_leafPos)
		delete[] _leafPos;
	if (_leafScale)
		delete[] _leafScale;
...
{
	_numCheckImages = depthchecks;
	if (_numCheckImages < 0)
		_numCheckImages = 0;
...
bool FusionMipMapCPU::copyCPU2HDD()
{
	...
	if (_nBranchesUsed)
		file.write((char *)_tree, _nBranchesUsed * sizeof(volumetype));
	if (_nLeavesUsed)
	{
		...
		if (_useColor)
			file.write((char *)_color, _nLeavesUsed * sizeof(colortype3) * _brickSize);
	}
	file.close();
...
bool FusionMipMapCPU::copyHDD2CPU()
{
	...
	if (_nBranchesUsed)
		file.read((char *)_tree, _nBranchesUsed * sizeof(volumetype));
	if (_nLeavesUsed)
	{
		...
		if (_useColor)
			file.read((char *)_color, _nLeavesUsed * sizeof(colortype3) * _brickSize);
	}
	file.close();
	...
	for (int y = 0; y < imHeight; y++)
	{
		for (int x = 0; x < imWidth; x++)
		{
			...
			if (TRAVERSALHEIGHTFINITECPU)
			{
				...
				if (*minX > vx - bandwidth)
					*minX = vx - bandwidth;
				if (*minY > vy - bandwidth)
					*minY = vy - bandwidth;
				if (*minZ > vz - bandwidth)
					*minZ = vz - bandwidth;

				if (*maxX < vx + bandwidth)
					*maxX = vx + bandwidth;
				if (*maxY < vy + bandwidth)
					*maxY = vy + bandwidth;
				if (*maxZ < vz + bandwidth)
					*maxZ = vz + bandwidth;
			}
			else
			{
							...
							std::vector<cv::Mat> rgb)
{
	...
	for (volumetype i = 0; i < _nLeavesTotal; i++)
		_queueIndexOfLeaf[i] = MAXLEAFNUMBER;
	_nLeavesQueuedSurface = 0;
	...
	{
		_imageWidth = depth.cols;
		_imageHeight = depth.rows;
		if (_sharedBoxes)
			delete[] _sharedBoxes;
	...
	if (_threaded)
	{
		distanceUpdateThread = new boost::thread(updateWrapperFloat, SDFUpdateParameterFloat((const float *)data, (const uchar *)rgb[2].data, (const uchar *)rgb[1].data, (const uchar *)rgb[0].data, _imageWidth, _imageHeight, m11, m12, m13, m14, m21, m22, m23, m24, m31, m32, m33, m34, pInv.fx, pInv.fy, pInv.cx, pInv.cy, _scale, _distanceThreshold, _leafNumberSurface, _leafPos, _leafScale, _distance, _weights, _color, _brickLength), &_nLeavesQueuedSurface, &_threadValid, 0);
	}
	...
	transformLoopSimPrecalculatedNeg_vis(qxp1, qxp2, qxp3, qyp1, qyp2, qyp3, p.t1, p.t2, p.t3,
										 _n, _bandwidth, _brickLength, _imageWidth, _imageHeight, _boxMin, _boxMax, data,
										 _tree, _nBranchesUsed, _nLeavesTotal, _nLeavesUsed, _nLeavesQueuedSurface,
										 _leafNumberSurface, _leafPos, _leafScale, _queueIndexOfLeaf, _child, _branchNumber, _meshCells, _meshCellIndicesBranch, _meshCellIndicesLeaf, _leafParent, _boundary, _performIncrementalMeshing);
	...
	else
	{
		...
		updateWrapperFloat(SDFUpdateParameterFloat(
							   ...
							   _distance, _weights, _color, _brickLength),
						   &_nLeavesQueuedSurface, &_threadValid, 0);
							...
							float scaling, float maxcamdistance)
{
	...
	if (_threaded)
	{
		distanceUpdateThread = new boost::thread(updateWrapperInteger, SDFUpdateParameterInteger((const ushort *)depthdata, scaling, maxcamdistance, (const uchar *)rgb.data, _imageWidth, _imageHeight, m11, m12, m13, m14, m21, m22, m23, m24, m31, m32, m33, m34, pInv.fx, pInv.fy, pInv.cx, pInv.cy, _scale, _distanceThreshold, _leafNumberSurface, _leafPos, _leafScale, _distance, _weights, _color, _brickLength), &_nLeavesQueuedSurface, &_threadValid, 0);
	}
	...
	const sidetype *_leafScale)
{
	...
	if (index >= MAXCELLINDEX)
		fprintf(stderr, "\nERROR: Interior Index in MCNCompact is wrong at [%i %i]: %li!", branch, childoffset, index);
	if (index >= _meshCellsCompact.size())
	{
		fprintf(stderr, "\nERROR: Wrong Index!");
		char c;
		std::cin >> c;
...
void populate_branch_separate(
	...
	volumetype leaf,
	const sidetype3 *_leafPos, const sidetype *_leafScale, FusionMipMapCPU::MeshCellArray &_meshCells, FusionMipMapCPU::BranchIndicesArray &_meshCellIndicesBranch, FusionMipMapCPU::LeafNeighborhood &meshCellIndicesLeafSingle, volumetype _treeSizeForMeshing)
{
...
void populate_branch_separate_compact(
	...
	sidetype half, sidetype _brickLength,
	volumetype leaf,
	const sidetype3 *_leafPos, const sidetype *_leafScale, FusionMipMapCPU::MeshCellArray &_meshCells, FusionMipMapCPU::BranchIndicesCompactArray &_meshCellIndicesBranch, FusionMipMapCPU::LeafNeighborhood &meshCellIndicesLeafSingle, volumetype _treeSizeForMeshing)
{
	for (uchar childoffset = 0; childoffset < 8; childoffset++)
	{
		if (half > _brickLength && _tree[branch + 8 + childoffset] < _treeSizeForMeshing)
...
//	}
	...
	uchar maxLevel = 0;
	for (sidetype length = _brickLength; length < _n / 2; length *= 2)
		maxLevel++;
	for (size_t i = 0; i < _newBudsSinceMeshingToClear.subtreeBuds->size(); i++)
	{
		...
		updateMeshCellStructureBranches_recursive_splitcompactdebug(_meshCellsSplit, _meshCellIndicesBranch, _boundary,
																	_meshCellsCompact, _meshCellIndicesBranchCompact, _meshCellIndicesLeaf, _boundaryCompact, _tree,
		...
		updateMeshCellStructureBranches_recursive(_meshCellsSplit, _meshCellIndicesBranch, _meshCellIndicesLeaf, _boundary, _tree,
												  (*_newBudsSinceMeshingToClear.subtreeBuds)[i],
												  (*_newBudsSinceMeshingToClear.subtreeBudsParentLeaf)[i],
												  _n, _brickLength, _treeSizeForMeshing, _leafScale);
//		fprintf(stderr,"\nBranch Indices from Split for bud %i updated",(*_newBudsSinceMeshingToClear.subtreeBuds)[i]);
		...
		updateMeshCellStructureBranches_recursive_compact(_meshCellsCompact, _meshCellIndicesBranchCompact, _meshCellIndicesLeaf, _boundaryCompact, _tree,
														  (*_newBudsSinceMeshingToClear.subtreeBuds)[i],
														  (*_newBudsSinceMeshingToClear.subtreeBudsParentLeaf)[i],
														  _n, _brickLength, _treeSizeForMeshing, _leafScale);
	...
	for (std::deque<volumetype>::iterator it = _leafQueueForMeshing.begin(); it != _leafQueueForMeshing.end(); it++)
	{
		...
		for (LeafNeighborhood::iterator j = _meshCellIndicesLeaf[leaf].begin(); j != _meshCellIndicesLeaf[leaf].end(); j++)
		{
			...
			if (!(*_meshCellIsQueuedNext)[*j])
			{
				...
				if (cell.type == 0 || ((cell.type == 1 || cell.type == 2 || cell.type == 3) && (cell.lastLeaf[0] < BRANCHINIT && cell.lastLeaf[1] < BRANCHINIT)) || ((cell.type == 4 || cell.type == 5 || cell.type == 6) && (cell.lastLeaf[0] < BRANCHINIT && cell.lastLeaf[1] < BRANCHINIT && cell.lastLeaf[2] < BRANCHINIT && cell.lastLeaf[3] < BRANCHINIT)) || ((cell.type == 7) && (cell.lastLeaf[0] < BRANCHINIT && cell.lastLeaf[1] < BRANCHINIT && cell.lastLeaf[2] < BRANCHINIT && cell.lastLeaf[3] < BRANCHINIT && cell.lastLeaf[4] < BRANCHINIT && cell.lastLeaf[5] < BRANCHINIT && cell.lastLeaf[6] < BRANCHINIT && cell.lastLeaf[7] < BRANCHINIT)))
				{
	...
	std::vector<FusionMipMapCPU::MeshStatistic> *meshTimes)
{
	...
	*mesh = MeshInterleaved(3);
	...
	if (meshTimes)
		meshTimes->push_back(FusionMipMapCPU::MeshStatistic(0, oldSize, meshcellsSize, timeUpdate, timeSum));
	eprintf("\nMeshes summed up.");
	*meshingDone = 0;
}
	...
	if (_meshingDone == 0)
	{
		...
		if (_threadMeshing)
		{
			if (_meshThread)
			{
				...
				{
					if (_meshTimes.size())
						_meshTimes.back().frameNumber = _meshingStartFrame;
				}
				delete _meshThread;
				_meshThread = NULL;
			}
			//			MeshSeparate *separate = _meshSeparateCurrent; _meshSeparateCurrent = _meshSeparateNext; _meshSeparateNext = separate;
			MeshInterleaved *interleaved = _meshCurrent;
			_meshCurrent = _meshNext;
			_meshNext = interleaved;
...
CellUpdate &FusionMipMapCPU::getMeshCellsUpdate()
{
	...
	if (_threadMeshing)
	{
		...
		MeshSeparate *temp = _meshSeparateCurrent;
		_meshSeparateCurrent = _meshSeparateNext;
		_meshSeparateNext = temp;
...
bool FusionMipMapCPU::grow()
{
	...
	{
		if (xm > xp)
			xm = 2 * highestPowerOf2(xm) + (xm & (~cut));
		else
			xp = 2 * highestPowerOf2(xp) + (xp & (~cut));
	}
	cut = yp & ym;
	if (cut)
	{
		if (ym > yp)
			ym = 2 * highestPowerOf2(ym) + (ym & (~cut));
		else
			yp = 2 * highestPowerOf2(yp) + (yp & (~cut));
	}
	cut = zp & zm;
	if (cut)
	{
		if (zm > zp)
			zm = 2 * highestPowerOf2(zm) + (zm & (~cut));
		else
			zp = 2 * highestPowerOf2(zp) + (zp & (~cut));
	}
	//	fprintf(stderr,"\nDescriptors after processing: [%i %i %i]-[%i %i %i]",xm,ym,zm,xp,yp,zp);
	sidetype maxAddLevel = highestPowerOf2(xm);
	if (maxAddLevel < highestPowerOf2(xp))
		maxAddLevel = highestPowerOf2(xp);
	if (maxAddLevel < highestPowerOf2(ym))
		maxAddLevel = highestPowerOf2(ym);
	if (maxAddLevel < highestPowerOf2(yp))
		maxAddLevel = highestPowerOf2(yp);
	if (maxAddLevel < highestPowerOf2(zm))
		maxAddLevel = highestPowerOf2(zm);
	if (maxAddLevel < highestPowerOf2(zp))
		maxAddLevel = highestPowerOf2(zp);
	...
	for (volumetype i = 0; i < maxAddLevel * 16; i++)
		_tree[i] = BRANCHINIT;
	delete[] treeTemp;
	...
	if (_performIncrementalMeshing)
	{
		...
		for (size_t i = 0; i < maxAddLevel * 16; i++)
		{
			...
			for (int j = 0; j < 12; j++)
				_meshCellIndicesBranch[i].indexEdgeCorner[j].clear();
			for (int j = 0; j < 8; j++)
				_meshCellIndicesBranch[i].indexCorner[j] = MAXCELLINDEX;
#endif
		}
		fprintf(stderr, "\nShifting Branch References in MeshCells...");
		for (size_t i = 0; i < _meshCellsSplit.size(); i++)
		{
			if (_meshCellsSplit[i].lastBranch[0] < BRANCHINIT)
				_meshCellsSplit[i].lastBranch[0] += maxAddLevel * 16;
			if (_meshCellsSplit[i].lastBranch[1] < BRANCHINIT)
				_meshCellsSplit[i].lastBranch[1] += maxAddLevel * 16;
			if (_meshCellsSplit[i].lastBranch[2] < BRANCHINIT)
				_meshCellsSplit[i].lastBranch[2] += maxAddLevel * 16;
			if (_meshCellsSplit[i].lastBranch[3] < BRANCHINIT)
				_meshCellsSplit[i].lastBranch[3] += maxAddLevel * 16;
			if (_meshCellsSplit[i].lastBranch[4] < BRANCHINIT)
				_meshCellsSplit[i].lastBranch[4] += maxAddLevel * 16;
			if (_meshCellsSplit[i].lastBranch[5] < BRANCHINIT)
				_meshCellsSplit[i].lastBranch[5] += maxAddLevel * 16;
			if (_meshCellsSplit[i].lastBranch[6] < BRANCHINIT)
				_meshCellsSplit[i].lastBranch[6] += maxAddLevel * 16;
			if (_meshCellsSplit[i].lastBranch[7] < BRANCHINIT)
				_meshCellsSplit[i].lastBranch[7] += maxAddLevel * 16;
		}
#endif
#ifdef MESHCELLINDICES_COMPACT
		...
		{
			if (_meshCellsCompact[i].lastBranch[0] < BRANCHINIT)
				_meshCellsCompact[i].lastBranch[0] += maxAddLevel * 16;
			if (_meshCellsCompact[i].lastBranch[1] < BRANCHINIT)
				_meshCellsCompact[i].lastBranch[1] += maxAddLevel * 16;
			if (_meshCellsCompact[i].lastBranch[2] < BRANCHINIT)
				_meshCellsCompact[i].lastBranch[2] += maxAddLevel * 16;
			if (_meshCellsCompact[i].lastBranch[3] < BRANCHINIT)
				_meshCellsCompact[i].lastBranch[3] += maxAddLevel * 16;
			if (_meshCellsCompact[i].lastBranch[4] < BRANCHINIT)
				_meshCellsCompact[i].lastBranch[4] += maxAddLevel * 16;
			if (_meshCellsCompact[i].lastBranch[5] < BRANCHINIT)
				_meshCellsCompact[i].lastBranch[5] += maxAddLevel * 16;
			if (_meshCellsCompact[i].lastBranch[6] < BRANCHINIT)
				_meshCellsCompact[i].lastBranch[6] += maxAddLevel * 16;
			if (_meshCellsCompact[i].lastBranch[7] < BRANCHINIT)
				_meshCellsCompact[i].lastBranch[7] += maxAddLevel * 16;
		}
#endif
...
void FusionMipMapCPU::deleteOldLoopPoses()
{
	...
	for (std::multimap<float, size_t>::iterator i = _posemap.begin(); i != _posemap.end() && counter < _maxPosesPerLoop; i++, counter++)
	{
		...
		updateWrapper_deleteloop(threadparameterLoop(
									 ...
									 _distance, _weightsLoop, _color, _brickLength),
								 queue.size(), _leafMapLoop, _nLeavesTotal, &_nLeavesMapped);
...
// void FusionMipMapCPU::saveZimages(std::vector<volumetype> indices, std::string prefix)
{
	...
	for (volumetype i = 0; i < indices.size(); i++)
		indices[i] = i;
	...
	else
	{
		...
		for (volumetype i = 0; i < _nLeavesUsed * _brickSize; i++)
		{
			if (_weights[i] > 0)
			{
				if (distMin > _distance[i])
					distMin = _distance[i];
				if (distMax < _distance[i])
					distMax = _distance[i];
			}
		}
									...
									for (sidetype y1 = 0; y1 < _brickLength; y1++)
									{
										for (sidetype x1 = 0; x1 < _brickLength; x1++)
										{
											...
											else
											{
												...
												if (dist < 0)
												{
													source.r = 255.0f - dist / distMin * 255.0f;
													source.g = 0;
												}
												else if (dist == 0)
												{
												...
												{
													source.g = 255.0f - dist / distMax * 255.0f;
													source.r = 0;
												}
												colorsource.r = col.x / COLOR_MULTIPLICATOR;
												colorsource.g = col.y / COLOR_MULTIPLICATOR;
												colorsource.b = col.z / COLOR_MULTIPLICATOR;
											}
											//											for(sidetype z2=0;z2<scale;z2++){
											for (sidetype z2 = largMemOff; z2 < largMemOff + imagesInMemory; z2++)
											{
...
// void FusionMipMapCPU::saveZimages(std::vector<volumetype> indices, std::string prefix)
{
	...
	else
	{
		...
		for (long long i = 0; i < hugeSize; i++)
			memory[i] = -128;
		fprintf(stderr, "\nMemory Initialized");
						...
						for (sidetype z1 = 0; z1 < _brickLength; z1++)
						{
							for (sidetype y1 = 0; y1 < _brickLength; y1++)
							{
								for (sidetype x1 = 0; x1 < _brickLength; x1++)
								{
									...
									char source = -127;
									if (weight > 0)
										source = (char)(dist / distMax * 125.0f);
									//									fprintf(stderr,"W");
									for (sidetype z2 = 0; z2 < scale; z2++)
									{
										for (sidetype y2 = 0; y2 < scale; y2++)
		...
		invalid.r = invalid.g = 0;
		invalid.b = invalid.a = 255;
		bgra vertexcolor;
