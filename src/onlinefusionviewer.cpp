...
OnlineFusionViewerManipulated::OnlineFusionViewerManipulated(bool createMeshList) : QGLViewer(),
																					...
																					_runFusion(false), _createMeshList(createMeshList), _lightingEnabled(false), _colorEnabled(true), _lightingInternal(false), _timer(NULL), _buffersGenerated(false)
{
	_meshNumber = 0;
	_fusionNumber = 0;
	_cx = 0.0f;
...
OnlineFusionViewerManipulated::~OnlineFusionViewerManipulated()
{
	...
	_currentMeshInterleaved->writePLY(saveFileName, true);
...
void OnlineFusionViewerManipulated::enableLighting()
{
	_lightingInternal = true;
	glColor3f(0.5, 0.5, 0.5);
	glEnable(GL_LIGHTING);
}
void OnlineFusionViewerManipulated::disableLighting()
{
...
void OnlineFusionViewerManipulated::drawGridFrame(float ox, float oy, float oz, float sx, float sy, float sz)
{
	...
	glBegin(GL_LINES);
	glVertex3f(ox, oy, oz);
	glVertex3f(ox + sx, oy, oz);
	glVertex3f(ox, oy + sy, oz);
	glVertex3f(ox + sx, oy + sy, oz);
	glVertex3f(ox, oy, oz + sz);
	glVertex3f(ox + sx, oy, oz + sz);
	glVertex3f(ox, oy + sy, oz + sz);
...
void OnlineFusionViewerManipulated::drawMeshPointer()
{
	...
	{
		if (_pointermeshes[drawFrame] != _currentMesh || _pointermeshes[drawFrame]->type != _currentMeshType || _pointermeshes[drawFrame]->nv != _currentNV || _pointermeshes[drawFrame]->nf != _currentNF)
		{
			if (!_buffersGenerated)
			{
				generateBuffers();
	...
	std::string imageName)
{
	...
	{
		if (_verbose)
			fprintf(stderr, "\nDrawing Points from1 %s", imageName.c_str());
		cv::Mat depthImage = cv::imread(imageName, 2);
		depthImage.convertTo(depthImage, CV_32FC1, 1 / DEPTHSCALE);
		if (depthImage.empty())
			fprintf(stderr, "\nERROR: DepthIMage empty2");
		glPointSize(2.0);
		glColor3f(1, 0, 0);
		glBegin(GL_POINTS);
...
void OnlineFusionViewerManipulated::setScenePosition(CameraInfo pose)
{
	...
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			m[i][j] = inv.getRotation().at<double>(i, j);
	qglviewer::Quaternion orientation;
	orientation.setFromRotationMatrix(m);
...
void OnlineFusionViewerManipulated::keyPressEvent(QKeyEvent *e)
{
	...
	else if ((e->key() == Qt::Key_K) && (modifiers == Qt::NoButton)) // Showing Depth Image
	{
	...
	else if ((e->key() == Qt::Key_P) && (modifiers == Qt::NoButton)) // Display Mode is Faces
	{
	...
	else if ((e->key() == Qt::Key_D) && (modifiers == Qt::NoButton)) // Saving a bunch of Debug Images
	{
	...
	if (_threadFusion)
	{
		...
		if (_newMesh)
		{
			...
			if (_createMeshList)
			{
				...
				if (_pointermeshes[_currentFrame])
					delete _pointermeshes[_currentFrame];
				_pointermeshes[_currentFrame] = new PointerMeshDraw(_fusion->getMeshInterleavedMarchingCubes(), _lightingEnabled ? 1 : 0);
			}
			else
			{
				if (!_pointermeshes.size())
					_pointermeshes.resize(1, NULL);
				if (_pointermeshes[0])
					delete _pointermeshes[0];
				if (!_currentMeshForSave)
					_currentMeshForSave = new MeshSeparate(3);
				if (!_currentMeshInterleaved)
				...
				*_currentMeshInterleaved = _fusion->getMeshInterleavedMarchingCubes();
				if (_lightingEnabled)
					_pointermeshes[0] = new PointerMeshDraw(*_currentMeshInterleaved, _lightingEnabled ? 1 : 0);
#endif
#endif
